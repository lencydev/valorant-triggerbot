...
use eframe::egui::{
  ...
  DragValue, Context, RichText, ScrollArea
};
  ...
  CentralPanel::default().show(ctx, |ui| {
    ...
    ui.vertical(|ui| {
      ...
      ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
          ui.label("Resolution:");
          let mut temp_resolution = app.settings.selected_resolution;
          ComboBox::from_id_source("resolution_combo")
            .selected_text(format!("{}", app.settings.selected_resolution))
            .width(150.0)
            .show_ui(ui, |ui| {
              ...
              ScrollArea::vertical().max_height(80.0).show(ui, |ui| {
                for resolution in app.get_available_resolutions() {
                  ui.selectable_value(&mut temp_resolution, resolution.clone(), resolution.to_string());
                }
              });
            });
          if temp_resolution != app.settings.selected_resolution {
            app.settings.selected_resolution = temp_resolution;
